...
- Objectif du chantier : intégrer techniquement les sources RSS simples validées par l'audit légal du 2026-05-20, en gardant l'export public strictement whitelisted.
- État initial : `git status --short` montrait déjà `M antenne_radio/05_plan_amelioration.md` et `M antenne_radio/LEGAL_AUDIT.md`, changements préexistants non revertis.
- Chemins vérifiés avant modification : `antenne_radio/config/sources.yaml`, `antenne_radio/scripts/export/export_public.py`, `antenne_radio/tests/test_config.py`, `antenne_radio/tests/test_export_public.py`, `antenne_radio/01_RESSOURCES_SUIVIES.md`, `antenne_radio/data/logs/api.log`, `antenne_radio/data/logs/pipeline.log`, `static/antenne-radio/index.json`.
- Fichiers modifiés : `config/sources.yaml`, `scripts/export/export_public.py`, `tests/test_config.py`, `tests/test_export_public.py`, `01_RESSOURCES_SUIVIES.md`, `codex_memoire_materielle.md`.
- Artefacts régénérés : `data/raw/rss_latest.json`, `data/raw/hal_latest.json`, `data/raw/crossref_latest.json`, `data/normalized/db.json`, `data/exports/veille-2026-21.md`, `static/antenne-radio/index.json`, `data/logs/api.log`, `data/logs/pipeline.log`.
- Sources RSS activées après validation finale : Radiomorphoses, Radio Fañch, Les Radios Libres, La Radio du Futur, La Lettre Pro de la Radio, MeCCSA Radio & Audio Studies, Nieman Storyboard. Sources actives préexistantes conservées : Radio Survivor, Journal of Radio & Audio Media, Sounding Out! blog. HAL reste actif ; Crossref reste désactivé.
- Transom : l'audit légal le valide pour métadonnées, mais le run technique retrouve statut 301, warning feedparser et 0 entrée sur `https://transom.org/feed/`; il est donc déclaré avec `enabled: false` et reporté, conformément à la règle d'incertitude.
- Attributions publiques ajoutées dans `AUDITED_ATTRIBUTIONS` : `radiomorphoses`, `radio_fanch`, `les_radios_libres`, `la_radio_du_futur`, `la_lettre_pro`, `meccsa_radio_audio_studies`, `nieman_storyboard`, `transom`. Le mapping par `source_name` a été ajouté sans élargir la whitelist publique.
- Tests ajoutés : `test_legal_audit_2026_05_20_rss_sources_are_configured` vérifie les sources activées/reportées ; `test_public_export_maps_legal_audit_2026_05_20_sources` vérifie les nouvelles attributions publiques.
- Commandes lancées : `git status --short`; lectures de `docs/AGENTS.md`, de cette mémoire, de `LEGAL_AUDIT.md`, `sources.yaml`, `export_public.py`, `01_RESSOURCES_SUIVIES.md` et des tests ; `make test`; `make run` avec réseau ; inspection de `api.log` et `pipeline.log`; `make export-public`; contrôles `jq` et `rg` anti-fuite sur `static/antenne-radio/index.json`.
- Résultats tests : `make test` passe avec 81 tests sous Python 3.14.5 / pytest 9.0.3.
- Run final du 2026-05-20 10:15 JST : pipeline OK, `failed_steps=none`, aucune nouvelle ligne dans `api.log`, RSS `entry_count=239` avec `errors=[]`, HAL `result_count=20` et `num_found=931`, Crossref `result_count=0`.
- Compteurs RSS du run final : Radio Survivor 52, Radiomorphoses 10, Radio Fañch 25, Les Radios Libres 10, La Radio du Futur 10, La Lettre Pro 20, MeCCSA 10, Nieman Storyboard 10, Journal of Radio & Audio Media 42, Sounding Out! 50.
- Compteurs persistés : `db.json` contient 282 items (`to_read=140`, `candidate=86`, `ignored=56`) ; premier run d'intégration a ajouté 95 items, puis le run final après désactivation Transom est idempotent (`added_count=0`, `saved_count=282`).
- Export public : `make export-public` génère 226 items dans `static/antenne-radio/index.json`, contre 152 avant ce chantier. Les nouvelles sources visibles publiquement comptent : Radiomorphoses 9, Radio Fañch 21, Les Radios Libres 9, La Radio du Futur 4, La Lettre Pro 14, MeCCSA 9, Nieman Storyboard 8. Transom publie 0 item.
- QA anti-fuite : les items publics gardent seulement `id`, `title`, `url`, `doi`, `published_at`, `source_name`, `source_type`, `language`, `source_family`, `attribution_id`; scans sans match pour clés interdites, chemins locaux, secrets, `CROSSREF_MAILTO`, auteurs, tags, raw, abstracts, scores ou logs.
- Limites restantes : Transom reste à revalider avec un endpoint RSS réellement stable ; les résumés/raw restent des données privées du pipeline et ne sont pas publiés ; aucun build Hugo/browser pass n'a été relancé dans cette passe ; aucun cron, auto-commit, scraping, OpenAlex, CiNii, NDL, J-STAGE ou LLM n'a été ajouté.
- Prochaine étape recommandée : si l'on poursuit la v2, faire une QA Hugo légère de `/antenne-radio/` avec le JSON passé à 226 items, puis travailler l'ergonomie de tri/filtrage sans élargir les champs publics.

## 2026-05-20 - Prompt 6 v2 refonte template Hugo no-JS

- Objectif du chantier : remplacer l'ancien affichage tableau de `/antenne-radio/` par une grille de cartes neo-brutalistes premium, avec contrôles de filtrage JS et fallback statique accessible sans JavaScript.
- État initial : `git status --short` était vide au tout début de ce prompt ; aucune ingestion live, aucun `make run`, aucun cron, aucun auto-commit et aucune régénération d'export public n'ont été lancés.
- Chemins vérifiés avant modification : `antenne_radio/` comme module réel ; template existant `../layouts/antenne-radio/list.html` ; page `../content/antenne-radio/_index.md` ; JSON public existant `../static/antenne-radio/index.json`.
- Fichiers modifiés : `../layouts/antenne-radio/list.html` et ce fichier `codex_memoire_materielle.md`.
- Changement template : lecture Hugo build-time du JSON public via `readFile` / `transform.Unmarshal`, en-tête avec compteurs et date `generated_at`, formulaire avec IDs descriptifs `antenne-radio-filter-category`, `antenne-radio-filter-source`, `antenne-radio-filter-language`, `antenne-radio-filter-search`, grille `antenne-radio-card-grid` alimentée par JS, et section d'attributions légales/éthiques en bas de page.
- Fallback no-JS : balise `<noscript>` ajoutée avec `range first 50 $items`, cartes accessibles en HTML statique pur et données strictement issues de `static/antenne-radio/index.json`.
- CSS : palette Catppuccin du site, bordures épaisses, ombres franches ; transition des cartes strictement limitée à `transform 0.2s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.2s cubic-bezier(0.4, 0, 0.2, 1)` ; aucun `transition: all`, `transition-all`, `backdrop-filter`, `blur()`, `drop-shadow` ou `will-change` ajouté dans le template.
- Commandes lancées : `git status --short` ; lectures de `docs/AGENTS.md`, de cette mémoire, du template, de `_index.md`, du JSON public et de la skill Hugo locale ; repérage `rg --files antenne_radio layouts/antenne-radio content/antenne-radio static/antenne-radio assets`; builds `pnpm exec hugo --gc --minify --cleanDestinationDir --logLevel info --printPathWarnings` ; scans `rg` ciblés anti-transition et anti-fuite ; contrôles Node sur le JSON public et le bloc `<noscript>` ; `git diff --check -- layouts/antenne-radio/list.html` ; serveur local `make dev` ; vérification navigateur in-app desktop/mobile.
- Résultats build : Hugo global passe avec Hugo `0.160.1`, 83 pages générées ; seul warning observé, déjà connu, sur `module.register()` côté Node/Tailwind et avertissement pnpm sur le champ `pnpm` de `package.json`.
- Résultats données publiques : `static/antenne-radio/index.json` reste à 226 items et 11 sources ; les clés item vérifiées restent seulement `attribution_id`, `doi`, `id`, `language`, `published_at`, `source_family`, `source_name`, `source_type`, `title`, `url`.
- Résultats no-JS : le HTML généré contient bien un `<noscript>` et 50 `<article class=antenne-radio-card>` statiques dans ce bloc.
- Résultats navigateur JS actif : `/antenne-radio/` affiche 226 cartes, compteur `226 signaux affichés sur 226`, les contrôles sont hydratés avec 3 options de catégorie, 12 options source et 4 options langue ; la recherche `HAL` filtre à 37 cartes depuis `HAL open archive`.
- Résultats mobile : viewport 390x844 vérifié sans débordement horizontal (`scrollWidth=390`, `clientWidth=390`) ; les contrôles passent sur une colonne et les 226 cartes restent rendues.
- Limites restantes : la capture screenshot navigateur a expiré côté outil, donc la preuve visuelle archivée repose sur les mesures DOM et non sur une image ; la section source listée dans `content/antenne-radio/_index.md` n'a pas été actualisée pour mentionner les nouvelles sources RSS, car le prompt était limité au template ; aucun test GitHub Actions distant n'a été lancé.
- Prochaine étape recommandée : faire une passe de microcopie/contenu sur `content/antenne-radio/_index.md` pour aligner la liste descriptive des sources avec les 11 sources publiques actuelles, puis seulement ensuite poursuivre l'ergonomie de tri/deep-linking si demandé.

## 2026-05-20 - Prompt 7 v2 JS vanilla filtres et rendu progressif

- Objectif du chantier : extraire le moteur JS de `/antenne-radio/` dans un fichier autonome léger, chargé seulement sur cette page, avec filtrage multi-critère en mémoire et rendu progressif par lots de 50.
- État initial : `git status --short` montrait les changements non commités du Prompt 6 (`../layouts/antenne-radio/list.html` et ce fichier mémoire). Ces changements ont été conservés et poursuivis, sans revert.
- Chemins vérifiés avant création : module réel `antenne_radio/` ; template `../layouts/antenne-radio/list.html` ; JSON public existant `../static/antenne-radio/index.json` ; loader Hugo `../layouts/partials/functions/js-loader.html` ; scripts existants dans `../assets/js/`.
- Fichiers modifiés ou créés : `../assets/js/antenne-radio.js` créé ; `../layouts/antenne-radio/list.html` modifié ; ce fichier `codex_memoire_materielle.md` mis à jour.
- Changement template : suppression du JSON embarqué `antenne-radio-public-data` et du moteur de rendu inline ; ajout de `data-antenne-radio-root`, `data-index-url="/antenne-radio/index.json"`, bouton explicite `#load-more`, et chargement conditionnel par `{{ partial "functions/js-loader.html" (slice "antenne-radio") }}` dans le bloc `scripts` du template.
- Script JS : vanilla pur, sans dépendance, sans CDN ; il charge `/antenne-radio/index.json` au démarrage, garde les items publics en mémoire, hydrate les filtres, applique une logique ET stricte sur recherche texte + catégorie + source + langue, puis rend le DOM depuis l'état mémoire.
- Pagination : rendu initial limité à 50 cartes ; bouton `#load-more` ajoute des lots de 50 ; aucun infinite scroll, aucun `IntersectionObserver`.
- Poids : `assets/js/antenne-radio.js` pèse 7649 octets non minifié ; l'asset Hugo compilé/fingerprinté pèse 4465 octets.
- Commandes lancées : `git status --short` ; lectures de `docs/AGENTS.md`, de cette mémoire, du template et du loader JS ; repérage `rg --files antenne_radio layouts/antenne-radio content/antenne-radio static/antenne-radio assets/js layouts/partials/functions`; `wc -c`; builds `pnpm exec hugo --gc --minify --cleanDestinationDir --logLevel info --printPathWarnings`; scans `rg` anti-effets lourds et anti-fuite ; contrôle du `<noscript>` et de `#load-more` via Node ; contrôle de référencement du JS dans `public/`; serveur local `make dev` ; vérification navigateur in-app.
- Résultats build : Hugo global passe avec Hugo `0.160.1`, 83 pages générées ; mêmes warnings connus seulement (`module.register()` Node/Tailwind et champ `pnpm` de `package.json`).
- Résultats chargement conditionnel : l'asset `public/js/antenne-radio.*.js` est référencé uniquement dans `public/antenne-radio/index.html`, pas sur les autres pages testées.
- Résultats HTML public : le HTML de `/antenne-radio/` ne contient plus de JSON public complet embarqué ; le `<noscript>` conserve 50 cartes statiques ; `#load-more` est présent ; l'URL chargée par JS est `/antenne-radio/index.json`.
- Résultats navigateur : chargement initial JS actif = 50 cartes DOM, bouton `Afficher plus de signaux` visible, compteur `50 signaux affichés sur 226 résultats`; après clic sur le bouton = 100 cartes ; recherche `HAL` = 37 cartes et bouton masqué ; filtres ET `journal_article` + `hal` + `fr` + recherche `HAL` = 16 cartes.
